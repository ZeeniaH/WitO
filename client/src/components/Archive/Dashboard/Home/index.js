					...
					<Grid container spacing={2} sx={{ p: "25px", justifyContent: "center" }}>
						{userFolders.map(({ data, docId }) => (
							<Grid
								...
								xs={6}
								md={2}
								onDoubleClick={() => navigate(`/dashboard/archive/${companyId}/folder/${docId}`)}
								// onDoubleClick={() => navigate(`/dashboard/archive/folder/${docId}`)}
								onClick={(e) => {
					...
					<Grid container spacing={2} sx={{ p: "25px", justifyContent: "center" }}>
						{filteredFiles.map(({ data, docId }) => (
							<Grid
								...
								xs={6}
								md={2}
								onDoubleClick={() => navigate(`/dashboard/archive/file/${docId}`)}
								onClick={(e) => {
									if (e.currentTarget.classList.contains("text-white")) {
										e.currentTarget.style.background = "#fff";
										e.currentTarget.classList.remove("text-white");
