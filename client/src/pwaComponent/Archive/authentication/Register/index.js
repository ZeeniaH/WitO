...
const Register = () => {
	...
	const handleSubmit = (e) => {
		...
		if (password !== confirmPassword) return toast.dark("Passwords donot match!");

		if (password.length < 8) {
			return toast.dark("Password must be of length 8 or more");
		}
		if (!/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,16}$/.test(password)) {
			return toast.dark("Password must have alteast a number and a special character!");
		}
				...
				<Col md="5" className="mx-auto">
					<Form onSubmit={handleSubmit}>
						...
						<Form.Group controlId="formBasicBtn" className="mt-3">
							<Button variant="primary" type="submit" className="form-control" block>
								Register
							</Button>
						</Form.Group>
