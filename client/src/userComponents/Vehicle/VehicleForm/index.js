						...
						<Grid container spacing={2}>
							<Grid item xs={12}>
								<Box className={styles.rightSideSection}>
									...
									<Box className={styles.vehicleHeading} component={"h1"}>{t('Add New Vehicle')}</Box>
											...
											<Formik
												...
												onSubmit={async (
													...
													if (downloadUrls.length === 0) {
														setErrors({ carImage: "At least one Vehicle image is required" });
														return;
													}
													dispatch(setLoading(true));
													...
													if (response.ok) {
														...
														setSubmitting(false);
														navigate(`/home/vehicle-list/${params.id}`);
													}
													dispatch(setLoading(false));
												}}
											>
